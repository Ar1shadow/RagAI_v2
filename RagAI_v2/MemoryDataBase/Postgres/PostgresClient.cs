...
internal sealed class PostgresClient : IDisposable, IAsyncDisposable
{
    ...
    public PostgresClient(PostgresConfig config, ILoggerFactory? loggerFactory = null)
    {
        config.Validate();
            ...
            await using (cmd.ConfigureAwait(false))
            {
                ...
                else
                {
                    cmd.CommandText = @$"
                        ...
                        text_results AS (
                            SELECT {columns}, ts_rank_cd(to_tsvector('french', {this._colContent},1), plainto_tsquery('french', @query)) AS {colTextScore}
                            FROM {tableName}
                            WHERE to_tsvector('french', {this._colContent}) @@ plainto_tsquery('french', @query)
                            LIMIT @limit
                        ),
